...
export class NeuralDream {
  ...
  private async initializeModel(): Promise<void> {
    try {
      ...
      this.isReady = true;
      log.debug('NEURAL DREAM: Model loaded successfully');
    } catch (error) {
   ...
   */
  private patternToNoteSequence(pattern: number[], trackIndex: number): mm.INoteSequence {
    const notes: mm.INoteSequence['notes'] = [];
   ...
   */
  private noteSequenceToPattern(noteSequence: mm.INoteSequence, trackIndex: number): number[] {
    ...
    notes.forEach(note => {
      ...
      const endTime = note.endTime ?? startTime + stepDuration;
      const step = Math.floor(startTime / stepDuration);
      if (step >= 0 && step < 32) {
  ...
  private applyDreamTransformation(trackIndex: number, oldPattern: number[], newPattern: number[]): void {
    const command = CommandFactory.mutation(trackIndex, oldPattern, newPattern, (idx: number, data: number[]) => {
      if (window.seq?.data) {
  ...
  private showDreamDialog(show: boolean, message: string = 'Dreaming...'): void {
    const dialog = document.getElementById('neuralDreamDialog') as HTMLDialogElement | null;

    if (!dialog) {
      if (!show) return;

      // Create dialog
      const newDialog = document.createElement('dialog') as HTMLDialogElement;
